...
def forward_pass_and_eval(
    ...
    if args.use_encoder:
        if args.unobserved > 0 and args.model_unobserved == 0:
            ...
            # ---- hack ---
            (b, n, steps, f) = data_encoder.shape
            _logits = []
            for step in range(1, steps-10):
                _data = data_encoder[:, :, step:step+10, :]
                #_data = torch.reshape(_data, (b, n, 1, f))
                _logits.append(encoder(_data, rel_rec, rel_send))
    else:
    ...
    _edges = []
    _prob = []
    for __logits in _logits:
        _edges.append(utils.gumbel_softmax(__logits, tau=args.temp, hard=hard))
        _prob.append(utils.my_softmax(__logits, -1))
    ...
    if args.global_temp:
        ...
        losses['temp_logprob'] = utils.get_uniform_logprobs(inferred_mean.flatten(), inferred_width.flatten(), temperatures)
    ...
    #################### MAIN LOSSES ####################
    ### latent losses ###
    loss_kl = [utils.kl_latent(args, _prob, log_prior, predicted_atoms) for _prob in _prob]
    #losses["loss_kl"] = utils.kl_latent(args, prob, log_prior, predicted_atoms)
    losses["loss_kl"] = torch.mean(torch.tensor(loss_kl, requires_grad=True))
    _accuracy = torch.tensor(0.0)
    _auroc = torch.tensor(0.0)
    for step in range(1, len(_logits)):
        _rel = relations[:, step, :]
        acc = utils.edge_accuracy(_logits[step], _rel)
        _accuracy = torch.add(_accuracy, acc)
        _auroc = torch.add(_auroc, utils.calc_auroc(_prob[step], _rel))
    #losses["acc"] = utils.edge_accuracy(logits, relations)
    losses["acc"] = torch.divide(_accuracy, len(_logits))
    losses["auroc"] = torch.divide(_auroc, len(_logits))

    ### output losses ###
    _outputs = torch.tensor(0.0)
    _mse = torch.tensor(0.0)
    cnt = 0.0
    for output in outputs:
        _outputs = torch.add(_outputs, utils.nll_gaussian(output, target, args.var))
        _mse = torch.add(_mse, F.mse_loss(output, target))
        cnt += 1
    losses["loss_nll"] = torch.divide(_outputs, cnt)
    losses["loss_mse"] = torch.divide(_mse, cnt)
    ...
    return losses, output, unobserved, _edges
